...
const variants: Record<Variant, string> = {
  ...
  secondary:      'bg-white text-slate-700 hover:bg-slate-50/80 border border-slate-200 disabled:opacity-50',
...
export default function Button({ variant = 'primary', size = 'sm', className = '', children, ...rest }: ButtonProps) {
  return (
    <button
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded font-medium transition-colors ${sizes[size]} ${variants[variant]} ${className}`}
      {...rest}
    >
      {children}
    </button>
