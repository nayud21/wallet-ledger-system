  ...
  return (
    <header className="h-11 shrink-0 bg-white border-b border-slate-200 flex items-center px-4 gap-3">
      ...
      <div className="ml-auto flex items-center gap-2">
        <button className="h-7 px-2 rounded border border-slate-200 hover:bg-slate-50 text-xs text-slate-600 flex items-center gap-1.5">
          <IconSearch className="w-3.5 h-3.5" />
          <span>Search</span>
          <span className="ml-2 font-mono text-[10px] px-1 py-0.5 bg-slate-100 rounded text-slate-500">⌘K</span>
        </button>
            ...
            <>
              ...
              <div className="absolute right-0 top-9 w-64 bg-white border border-slate-200 rounded-md py-1 z-20 shadow-sm">
                <div className="px-3 py-2 border-b border-slate-100">
                  <div className="text-[10px] uppercase tracking-wide text-slate-400">Switch operator</div>
                </div>
                {operators.map((o) => (
                  <button key={o.name} className="w-full px-3 py-1.5 flex items-center gap-2 hover:bg-slate-50">
                    <span className="w-6 h-6 rounded bg-slate-800 text-white text-[10px] font-semibold grid place-items-center font-mono">
                      {o.init}
                    </span>
                    <span className="text-xs text-slate-700 text-left flex-1">
                      <div className="font-medium">{o.name}</div>
                      <div className="text-[10px] text-slate-400">{o.role}</div>
                    </span>
