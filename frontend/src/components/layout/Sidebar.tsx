  ...
  return (
    <aside className="w-56 shrink-0 bg-white border-r border-slate-200 flex flex-col">
      <div className="h-11 px-3 flex items-center gap-2 border-b border-slate-200">
        <div className="w-6 h-6 rounded bg-gradient-to-br from-indigo-500 to-indigo-700 grid place-items-center text-white text-xs font-bold font-mono">
          M
        </div>
      ...
      <nav className="flex-1 p-2 space-y-0.5">
        <div className="px-2 pt-2 pb-1 text-xs uppercase tracking-wider text-slate-500 font-medium">
          Operations
        </div>
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-2 px-2 h-7 rounded text-xs font-medium transition-colors ${
                active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50/80'
              }`}
            >
        ...
        <div className="px-2 pt-4 pb-1 text-xs uppercase tracking-wider text-slate-500 font-medium">
          Configuration
        </div>
        {configItems.map((label) => (
          <button
            key={label}
            className="w-full flex items-center gap-2 px-2 h-7 rounded text-xs font-medium text-slate-600 hover:bg-slate-50/80 transition-colors"
          >
      ...
      <div className="p-2 border-t border-slate-200">
        <div className="px-2 py-1.5 rounded bg-slate-50 border border-slate-200 text-xs text-slate-600 leading-snug">
          <div className="flex items-center gap-1.5 mb-0.5">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
            <span className="font-medium text-slate-700">All systems normal</span>
          </div>
          <span className="font-mono text-xs text-slate-500">last check 32s ago</span>
        </div>
      </div>
    </aside>
  );
